...
class AdminContrller extends Controller
{
    ...
    {
        $this->AuthLogin();
        $Listemail = DB::select('SELECT * FROM email as a, user as b WHERE a.idUser = b.idUser');
        return view('page.all_email_register')->with('Listemail', $Listemail);
    }
    public function send_email_all()
    {
        ...
        foreach ($Listemail as $key) {
            $name = $key->Name;
            $emailTo = $key->email;
            Mail::send('email.test', compact('name'), function ($mail) use ($name, $emailTo) {
                $mail->subject('New Job In Website New Star');
                $mail->to($emailTo, $name);
            });
        }

        return redirect('/admin/danh-sach-email-dang-ky');
    }
}