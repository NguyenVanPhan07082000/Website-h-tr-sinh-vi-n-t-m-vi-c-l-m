...
<div class="position-center">
    ...
    @foreach ($edit as $key => $edit_value)
        <form role="form" action="{{URL::to('/admin/update-quangcao/'.$edit_value->idQuangcao)}}" method="POST" enctype="multipart/form-data">
            {{csrf_field()}}
            
            <div class="form-group">
                <label for="exampleInputPassword1">Slogan Quảng Cáo</label>
                <textarea rows="4" class="form-control" name="txtSlogan" id="txtSlogan" placeholder="Giới thiệu về cẩm nang..." required>{{$edit_value->Slogan}}</textarea>
            </div>
            <div class="form-group">
                <label for="exampleInputPassword1">Link</label>
                <input type="text" class="form-control" name="txtLink" value="{{$edit_value->Link}}" id="txtLink" placeholder="Nhập tiêu đề cẩm nang..." required>
            </div>
