...
@section('add-Admin')
    <div class="position-center">
        ...
        @foreach ($edit as $key => $edit_value)
            <form role="form" action="{{ URL::to('/admin/update-admin/' . $edit_value->idAdmin) }}" method="POST"
                enctype="multipart/form-data">
                {{ csrf_field() }}
                <div class="form-group">
                    <label for="exampleInputEmail1">Tài Khoản</label>
                    <input type="text" class="form-control" value="{{ $edit_value->User }}" name="txtTk" id="txtTk"
                        placeholder="Nhập tên tài khoản..." required>
                </div>
                <div class="form-group">
                    <label for="exampleInputPassword1">Mật Khẩu</label>
                    <input type="password" class="form-control" value="{{ $edit_value->Password }}" name="txtPass"
                        id="txtPass" placeholder="Nhập mật khẩu..." required>
                </div>

                <div class="form-group">
                    <label for="exampleInputEmail1">Họ Và Tên</label>
                    <input type="text" class="form-control" value="{{ $edit_value->HotenAdmin }}" name="txtTen"
                        id="txtTen" placeholder="Nhập tên người dùng..." required>
                </div>
                <div class="form-group">
                    <label for="exampleInputEmail1">Ngày Sinh</label>
                    <input type="date" class="form-control" value="{{ $edit_value->Ngaysinh }}" name="txtNgaysinh"
                        id="txtNgaysinh" required>
                </div>
                <div class="form-group">
                ...
                <div class="form-group">
                    ...
                    <input type="email" class="form-control" value="{{ $edit_value->Email }}" name="txtEmail"
                        id="txtEmail" placeholder="Nhập email..." required>
                </div>
                <div class="form-group">
                    <label for="exampleInputEmail1">Số Điện Thoại</label>
                    <input type="tel" class="form-control" pattern="[0-9]{10}" value="{{ $edit_value->SDT }}"
                        name="txtSDT" id="txtSDT" placeholder="Nhập số điện thoại..." required>
                </div>
                <div class="form-group">
                    <label for="exampleInputPassword1">Địa Chỉ</label>
                    <textarea rows="4" class="form-control" name="txtDiachi" id="txtDiachi" placeholder="Giới địa chỉ..."
                        required>{{ $edit_value->Diachi }}</textarea>
                </div>
                <div class="form-group">
