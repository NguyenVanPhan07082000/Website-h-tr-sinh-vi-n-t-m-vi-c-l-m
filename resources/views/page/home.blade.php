...
@section('header')
    <header class="header">
        ...
        <form class="header__text-box2" action="{{ URL::to('/tim-kiem') }}" method="POST">
            {{ csrf_field() }}
            <input type="text" name="chucvu" id="find_name" placeholder="Nhập chức vụ ứng tuyển..."
                    ...
                    @foreach ($jobnoibac as $key => $jobnb)
                        <div class="job job--1">
                            <div class="col-1-of-4">
                                <img src="{{ asset('public/fontend/img/company' . '/' . $jobnb->Hinhanh) }}"
                                    class="logo-company" alt="logo-company" />
                            </div>
                            <div class="col-3-of-4 job_info">
                                <p class="job_info--name_job">
                                    <a href="{{ URL::to('/info-job' . '/' . $jobnb->idJob) }}" class="job_info--a">
                                        {{ $jobnb->Nganhnghe }} </a>
                                </p>
                ...
                @foreach ($company as $key => $cp)
                    <div class="col-1-of-3 card">
                        <div class="card_side card_side--font">
                            <img src="{{ asset('public/fontend/img/company' . '/' . $cp->Hinhanh) }}"
                                class="company_main--logo_img" alt="logo_img" />
                        </div>
    ...
    <div class="camnan">
        <div class="review camnan_content">
            <div class="header-content">
                ...
                <div class="nav">
                    <i class="fas fa-chevron-left btnLeft"></i><i class="fas fa-chevron-right btnRight"></i>
                </div>
            </div>
            <div class="review-box">
                @foreach ($camnan as $key => $cn)
                    <div class="box">
                        <div class="image">
                            <img src="{{ asset('public/fontend/img/Camnan' . '/' . $cn->Hinhanh) }}" alt="img" />
                        </div>
                        <div class="name">{{ $cn->Tencamnan }}</div>
                        <a href={{ $cn->Link }} class="designation">{{ $cn->Tieude }}</a>
                        <div class="text">
                            {{ $cn->Gioithieu }}
                        </div>
                    </div>
