...
@section('add-Admin')
    <div class="position-center">
        ...
        @foreach ($edit as $key => $edit_value)
            <form role="form" action="{{ URL::to('/admin/update-camnang/' . $edit_value->idCamnan) }}" method="POST"
                enctype="multipart/form-data">
                {{ csrf_field() }}

                <div class="form-group">
                    <label for="exampleInputEmail1">Tên Cẩm Nang</label>
                    <input type="text" class="form-control" value="{{ $edit_value->Tencamnan }}" name="txtTen"
                        id="txtTen" placeholder="Nhập tên cẩm nang..." required>
                </div>
                <div class="form-group">
                    <label for="exampleInputPassword1">Tiêu Đề</label>
                    <input type="text" class="form-control" value="{{ $edit_value->Tieude }}" name="txtTieude"
                        id="txtTieude" placeholder="Nhập tiêu đề cẩm nang..." required>
                </div>
                <div class="form-group">
                    ...
                    <textarea rows="4" class="form-control" name="txtGioithieu" id="txtGioithieu" placeholder="Giới thiệu về cẩm nang..."
                        required>{{ $edit_value->Gioithieu }}</textarea>
                </div>
